...
# replace term here
TERM = "term_13s"
# set so it's faster to check if an item is in that list
already_scraped = set()
bad_ids = set()

if os.path.exists(f"{TERM}_course_evaluations.csv"):
    old_df = pd.read_csv(f"{TERM}_course_evaluations.csv")
    already_scraped = set(list(old_df["Class ID"]))

bad_links_file = f"{TERM}_bad_links.pkl"
if os.path.exists(bad_links_file):
    with open(bad_links_file, 'rb') as file:
        bad_ids = pickle.load(file)
...
#rotating proxies; omitted in github
proxy_list = []
...
def get_url_from_row(row):
    ...
    url = f"{TARGET_URL}/{REPORT_URL}/{section_abrv.lower()}{class_num}.{term.split("\n")[0].lower()}"
    return url
...
for index, row in tqdm(data.iterrows()):
    ...
    try:
        ...
        if class_id in already_scraped or class_id in bad_ids:
            continue
    except:
    ...
    if num_requests >= COOKIE_LASTING_TIME:
        break
